...
```html
<template>
  <bmap-gllass="map" :center="{lng: 116.404, lat: 39.915}" :zoom="15">
    <bmap-gl-label content="我爱北京天安门" :position="{lng: 116.404, lat: 39.915}" :labelStyle="{color: 'red', fontSize : '24px'}" title="Hover me"/>
  </bmap-gl
</template>
```
...
<doc-preview>
  <bmap-gllass="map" :center="{lng: 116.404, lat: 39.915}" :zoom="15">
    <bmap-gl-label content="我爱北京天安门" :position="{lng: 116.404, lat: 39.915}" :labelStyle="labelStyle" title="Hover me"/>
  </bmap-gl
</doc-preview>
</template>

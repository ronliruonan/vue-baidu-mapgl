...
```html
<bmap-gl :center="{lng: 116.403765, lat: 39.914850}" :zoom="11">
  <bmap-gl-view class="map"></bmap-gl-view>
  <bmap-gl-control :offset="{width: '10px', height: '10px'}">
    <bmap-gl-auto-complete v-model="keyword" :sugStyle="{zIndex: 1}">
      <search-field placeholder="请输入地名关键字"></search-field> <!-- 这里指代一个自定义搜索框组件 -->
    </bmap-gl-auto-complete>
  </bmap-gl-control>
  <bmap-gl-local-search :keyword="keyword" :auto-viewport="true" ></bmap-gl-local-search>
</bmap-gl>
...
<bmap-gl :center="{lng: 116.403765, lat: 39.914850}" :zoom="11">
  <bmap-gl-view class="map"></bmap-gl-view>
  <bmap-gl-control :offset="{width: 10, height: 10}">
    <bmap-gl-auto-complete v-model="keyword" :sugStyle="{zIndex: 1}">
